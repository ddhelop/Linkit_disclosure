...
const ProfileViewLicenseSkeleton = () => {
  return (
    <div className="flex w-full flex-col gap-5 border-y border-grey40 bg-white p-5 md:px-[2.75rem] md:py-[1.88rem] lg:rounded-xl lg:border">
      ...
      <div className="flex flex-col gap-2">
        {[1, 2].map((i) => (
          <motion.div
            key={i}
            className="flex flex-col gap-2 rounded-lg bg-grey10 px-6 py-4"
          ...
          >
            ...
            <div className="flex items-center gap-2">
              <motion.div
                className="h-5 w-28 rounded bg-gray-200"
                animate={{ opacity: [0.5, 1, 0.5] }}
                transition={{ repeat: Infinity, duration: 1.5 }}
              />
              <motion.div
...
export default ProfileViewLicenseSkeleton
