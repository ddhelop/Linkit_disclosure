...
export default function Login() {
  ...
  const KAKAO_REDIRECT_URI = `${process.env.LINKIT_REDIRECT_URL}/login/oauth2/callback/kakao`
  const KAKAO_AUTH_URL = `https://kauth.kakao.com/oauth/authorize?client_id=${KAKAO_REST_API_KEY}&redirect_uri=${KAKAO_REDIRECT_URI}&response_type=code`

  // naver_login
  const NAVER_REST_API_KEY = process.env.NEXT_PUBLIC_NAVER_CLIENT_ID
  const NAVER_REDIRECT_URI = `${process.env.LINKIT_REDIRECT_URL}/login/oauth2/callback/naver`
  ...
  const GOOGLE_REDIRECT_URI = `${process.env.LINKIT_REDIRECT_URL}/login/oauth2/callback/google`
