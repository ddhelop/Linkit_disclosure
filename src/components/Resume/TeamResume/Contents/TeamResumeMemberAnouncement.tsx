  ...
  return (
    <div className="flex w-full flex-col gap-[0.94rem] rounded-2xl bg-[#fff] px-4 py-4 shadow-resume-box-shadow sm:px-[2.06rem] sm:py-[1.38rem]">
      <div className="flex items-center gap-[0.56rem]">
        <span className="text-sm font-semibold text-grey100 sm:text-lg">팀원 공고</span>
      </div>

      <div className="w-full text-sm ">
        <span className="w-auto bg-grey10 px-3 py-1 text-sm sm:text-base">
          필요로 하는 팀원에 대한 공고 내용을 추가해주세요!
        </span>
      </div>

      <div className="flex flex-col gap-2">
        {announcements?.map((announcement) => (
          <div key={announcement.id} className="flex w-full flex-col gap-2">
            <div className="flex justify-between rounded-[0.63rem] border border-grey30 p-[1.25rem]">
              <div className="flex w-[90%] flex-col">
                <p className="text-sm text-grey60">{announcement.teamName}</p>
                <p className="whitespace-pre-wrap pt-[0.44rem] text-xs text-grey100 sm:text-base">
                  {announcement.mainBusiness}
                </p>
                ...
                <div className="flex flex-wrap gap-2">
                  {announcement.skillNames.map((skill, index) => (
                    <div
                      key={index}
                      className="mt-3 rounded-[0.51rem] border border-grey40 px-2 py-2 text-center text-xs text-grey60 sm:text-sm"
                    >
                      {skill}
                    </div>
                  ))}
            ...
            {isFormVisible && editingAnnouncementId === announcement.id && (
              <form onSubmit={handleSubmit(onSubmit)} className="mt-2 rounded border border-grey30 bg-grey10 p-4">
                <div className="flex flex-col gap-4">
                  <div>
                    <label className="flex font-normal text-grey100">
                      역할 <p className="pl-1 font-normal text-[#2563EB]">*</p>
                    </label>
                    <div className="mt-2 flex flex-wrap gap-2 text-xs sm:text-base">
                    ...
                    <div className="flex flex-wrap gap-2 text-xs sm:text-base">
                      {skills.map((skill, index) => (
                        <div
                          ...
                          className="flex cursor-pointer items-center rounded-lg border border-[#2563EB] bg-[#E0E7FF] px-2 py-1"
                        >
                  ...
                  <div className="">
                    ...
                    >
                      <p className="text-sm sm:text-base">요구 역량 선택</p>
      ...
      {!isFormVisible && editingAnnouncementId === null && (
